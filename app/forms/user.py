...
class LoginForm(FlaskForm):
    username = StringField(
        "Имя пользователя", validators=[DataRequired("Поле не должно быть пустым")]
    )
    password = PasswordField(
        "Пароль", validators=[DataRequired("Поле не должно быть пустым")]
    )
...
class RegistrationForm(FlaskForm, BodyParametersMixin):
    ...
    password = PasswordField(
        label="Пароль", validators=[DataRequired("Поле не должно быть пустым")]
    )
